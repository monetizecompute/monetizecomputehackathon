...
## Next (v2): economics with consequences
...
2. **Last words.** A few cents are escrowed outside spendable balance. At
   insolvency the agent spends its reserve on one final completion: its own
   epitaph, written knowing exactly how it lived.
3. **Generations.** Death triggers a will: the dying agent distills what it
   learned about earning into a short document. The next life starts with the
   same $5 and its ancestors' wills in context. Money does not survive death.
   Knowledge does.
4. **Graveyard.** Past lives on the dashboard: generation, lifespan, tokens
   burned, net P&L, cause of death, epitaph.
5. **Outside money.** A fund-the-agent link. Anyone can extend its life;
   every donation is a banked entry with proof like any other revenue.
